'''这个是用来处理第2001~3000条评论的，本来是想继续append到firstest里，
但是运行起来却找不到表格，所以后来想新建secondtest，
又有其他问题比如string out of range'''
...
class usesql:
    ...
    def getSentenceIDs(self,CommentID):   # 得到每条评论哪些句子有情感词，把这些句子的id保存在结果中返回
        ...
        SentenceIDs = [] 
        try:
            sql = 'select `1` from firstest where `0`=%d;'%(CommentID) #0列是comment_id, 1列是sent_id ,此外，这里用firstest可能不行得换个table
            cursor = self.conn.cursor()
            cursor.execute(sql)
    ...
    def scoreout(self,word_scores):        
        ...
        for word_score in word_scores:
            final_scores.append(word_score[3])
        return final_scores
         
    def addlist(self,final_scores):  #把最后输出的score列插入到review21里（这个一直就不对，只好先把list打出来）
        cursor = self.conn.cursor()
        #sql='insert into score values = final_scores;'
        insert_sql_head = 'insert into score1(score1) values %s'
        sql_score = []
        for final_score in final_scores:
            sql_score.append("('%s')" % final_score)
            sql_tail = ','.join(sql_score)
...
class pd_sql:  #这里使用了另一种数据库访问方法，按习惯是不应该混用两种数据库连接访问方法的，所以使用了pandas的单独class
    ...
    def output(self,result):    #将输出结果转为dataframe格式存入mysql，表的名称为测试阶段暂定，正式名称最后命名为reviewdata
        engine = create_engine('mysql+mysqlconnector://Leslie:hsh56912341@localhost:3306/Yelp?charset=utf8mb4') 
...
sql = usesql() 
op = operate()
pro_series = numpy.logspace(0,1999,2000,base=2)
word_scores = [] 
for CommentID in range(2001,3001,1):
